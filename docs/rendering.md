...
`content/html.py`, built on the standard library's parser: a survey of thirty
...
`feed/atom.py` unescapes them exactly once: a poster who typed `&amp;` literally
sends `&amp;amp;`, and going twice would silently rewrite what they wrote.

## Seeing it without a Beeb

```sh
uv run sextile render --page 82489493            # in colour, as the Beeb draws it
uv run sextile render --page 8 --form grid       # character and attribute layers
uv run sextile render --page 8 --form bytes      # the wire stream, as a hex dump
```
