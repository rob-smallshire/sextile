...
from calendar_viewdata import CalendarApplication
from sextile import Arrival, Page, PageAddress, PageRequest, UnknownPageError
...
@pytest.fixture
def app() -> CalendarApplication:
    return CalendarApplication(now=lambda: WHEN)


async def page_at(app: CalendarApplication, digits: str, arrival: Arrival | None = None) -> Page:
    page = await app.respond(
        PageRequest(address=PageAddress(digits), arrival=arrival or Arrival())
    )
...
class TestThePages:
    ...
    async def test_every_page_number_is_answered(
        self, digits: str, app: CalendarApplication
    ) -> None:
    ...
    async def test_a_number_the_service_has_not_got_is_not_answered(
        ...
        assert await app.respond(PageRequest(address=PageAddress(digits))) is None

    async def test_the_index_says_what_day_it_is(self, app: CalendarApplication) -> None:
        assert "Sunday 02 August 2026" in text_of(await page_at(app, "1"))

    async def test_the_time_is_shown_to_the_second(self, app: CalendarApplication) -> None:
        assert "14:30:15" in text_of(await page_at(app, "2"))

    async def test_a_month_is_drawn_as_a_grid(self, app: CalendarApplication) -> None:
...
class TestMovingAboutTheYear:
    ...
    async def test_a_day_leads_to_its_own_month(self, app: CalendarApplication) -> None:
        page = await page_at(app, "4220260802")
        assert page.frames[0].destination("1") == PageAddress("3220260802")


class TestTheDaysToCome:
    async def test_the_menu_begins_today(self, app: CalendarApplication) -> None:
        page = await page_at(app, "4")
        assert page.destinations[0] == PageAddress("4220260802")

    async def test_it_runs_across_several_frames(self, app: CalendarApplication) -> None:
        #  Twenty-eight days will not fit nine to a frame.
        assert len((await page_at(app, "4")).frames) == 4

    async def test_the_sequence_runs_on_across_frames(self, app: CalendarApplication) -> None:
        page = await page_at(app, "4")
...
class TestSayingWhenThingsAre:
    ...
    async def test_a_day_says_how_far_off_it_is(
        self, day: date, expected: str, app: CalendarApplication
    ) -> None:
    ...
    async def test_a_leap_year_has_a_day_more(self, app: CalendarApplication) -> None:
...
class TestNamedJumps:
    ...
    def test_a_keyword_names_a_page(
        self, keyword: str, digits: str, app: CalendarApplication
    ) -> None:
        assert app.resolve(keyword) == PageAddress(digits)

    def test_a_word_that_is_no_keyword_names_nothing(self, app: CalendarApplication) -> None:
        with pytest.raises(UnknownPageError):
            app.resolve("BANANA")


class TestRingingOff:
    async def test_the_goodbye_page_drops_the_line(self, app: CalendarApplication) -> None:
        assert (await page_at(app, "90")).hang_up

    async def test_no_other_page_does(self, app: CalendarApplication) -> None:
...
class TestTheClock:
    async def test_the_service_reads_the_clock_each_time_it_is_asked(self) -> None:
        ...
        app = CalendarApplication(now=lambda: next(moments))
