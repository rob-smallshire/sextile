...
an instance to bind it to, so every service was writing the same three-line
passthrough three times over. These are those passthroughs, written once:
...
want the service's own log handed over, and the guide wants the keys only a
service can know, so those keep their thin wrappers where they are used.
"""
