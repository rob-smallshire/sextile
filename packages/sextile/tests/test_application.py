...
class TestMountingAsPartOfWhatAServiceIs:
    """A mount is a range of page numbers somebody else answers.

    It belongs in the same list as the pages, for the reason everything else
    ended up there: a service assembled in two calls has an order, and an
    order is something to get wrong.
    """

    async def test_a_mount_may_be_given_to_the_constructor(self) -> None:
        inner = Sextile(pages=[PageRoute("82{n:int}", _nothing, name="post")])
        app = Sextile(pages=[PageRoute("1", _nothing, name="main"), Mount("8", inner)])
        assert await app.ask("82489493") is not None

    async def test_and_is_the_same_thing_as_mounting_afterwards(self) -> None:
        inner = Sextile(pages=[PageRoute("82{n:int}", _nothing, name="post")])
        app = Sextile()
        app.mount("8", inner)
        assert await app.ask("82489493") is not None

    async def test_the_longest_prefix_still_wins(self) -> None:
        general = Sextile(pages=[PageRoute("8{n:int}", _nothing, name="general")])
        special = Sextile(pages=[PageRoute("82{n:int}", _nothing, name="special")])
        app = Sextile(pages=[Mount("8", general), Mount("82", special)])
        found = app.route(PageAddress("82489493"))
        assert found is not None and found.name == "special"


class TestAServiceAssembledFromMountsDescribingItself:
    """The contents and the keywords have to see the whole service.

    Otherwise a service in two halves has a contents page listing one of them,
    which is worse than having none: a reader believes it.
    """

    def _assembled(self) -> Sextile:
        inner = Sextile(
            pages=[
                PageRoute("82{post_id:int}", _nothing, name="post", title="One post"),
                PageRoute("8", _nothing, name="posts", title="Latest posts",
                          keywords=("LATEST",)),
            ]
        )
        return Sextile(
    ...
    def test_a_mounted_page_is_listed(self) -> None:
        titles = {about.title for about in self._assembled().pages()}
        assert {"Main index", "Latest posts", "One post"} <= titles

    def test_the_service_s_own_pages_come_first(self) -> None:
        #  A reader meets the service before it meets whatever it is made of.
        titles = [about.title for about in self._assembled().pages()]
        assert titles[0] == "Main index"

    def test_a_mounted_keyword_is_offered(self) -> None:
        assert "LATEST" in self._assembled().keywords()

    def test_and_can_be_keyed(self) -> None:
        assert self._assembled().resolve("LATEST") == PageAddress("8")

    def test_a_word_the_service_itself_uses_is_not_overruled(self) -> None:
        #  A mount may not quietly take over a word its host has spoken for.
        inner = Sextile(pages=[PageRoute("82", _nothing, name="inner",
                                         keywords=("MAIN",))])
        app = Sextile(
            pages=[
                PageRoute("1", _nothing, name="main", keywords=("MAIN",)),
                Mount("8", inner),
            ]
        )
        assert app.resolve("MAIN") == PageAddress("1")

    def test_a_mounted_page_can_be_named_in_a_history(self) -> None:
        #  `describe` reads the route, and the route was on the far side of the
        #  mount -- so a history of a mounted service used to be a list of bare
        #  numbers.
        assert self._assembled().describe(PageAddress("82489493")) == "One post 489493"


class TestAMountedServiceIsStillItsOwnService:
    """What a mounted page is handed is its own, not its host's.

    Found by mounting two real services together, which nothing had done: both
    held something, and the mounted one was handed the host's. Its pages then
    could not reach what its own lifespan had opened, and its numbering was the
    host's numbering, so any address it built named one of the host's pages.
    """

    def _inner(self) -> Sextile:
        @asynccontextmanager
        async def lifespan(app: Sextile) -> AsyncIterator[dict[str, object]]:
            yield {"held": "the mounted service's own"}
        ...
        return Sextile(name="Inner", lifespan=lifespan,
                       pages=[PageRoute("82", page, name="inner")])

    async def test_it_is_handed_what_its_own_lifespan_opened(self) -> None:
        _seen.clear()
        inner = self._inner()
        await inner.startup()
        outer = Sextile(name="Outer", pages=[Mount("8", inner)])
        await outer.startup()
        await outer.ask("82")
        assert _seen == [("the mounted service's own", "Inner")]

    async def test_and_not_its_host_s(self) -> None:
        @asynccontextmanager
        async def host_lifespan(app: Sextile) -> AsyncIterator[dict[str, object]]:
            yield {"held": "the host's"}

        _seen.clear()
        inner = self._inner()
        await inner.startup()
        outer = Sextile(name="Outer", lifespan=host_lifespan, pages=[Mount("8", inner)])
        await outer.startup()
        await outer.ask("82")
        assert _seen == [("the mounted service's own", "Inner")]

    async def test_a_page_of_the_host_s_still_gets_the_host_s(self) -> None:
        @asynccontextmanager
        async def host_lifespan(app: Sextile) -> AsyncIterator[dict[str, object]]:
            yield {"held": "the host's"}
        ...
        outer = Sextile(
            ...
            pages=[PageRoute("1", _remembering, name="main"), Mount("8", inner)],
        )
        await outer.startup()
        await outer.ask("1")
        assert _seen == [("the host's", "Outer")]
...
class TestAMountedPageTheHostAnswersItself:
    """Mounts are consulted after this service's own pages, so a mounted page
    whose numbers the host already answers cannot be reached.

    Listing it anyway gives a contents page two entries for one number, only
    one of which does anything -- and a reader has no way to tell which.
    """

    def _both(self) -> Sextile:
        inner = Sextile(
            pages=[
                PageRoute("1", _nothing, name="inner_main", title="The index"),
                PageRoute("2", _nothing, name="clock", title="The time now"),
            ]
        )
        return Sextile(
            pages=[
                PageRoute("1", _nothing, name="main", title="Main menu"),
                Mount("", inner),
            ]
        )

    def test_the_shadowed_one_is_not_listed(self) -> None:
        titles = [about.title for about in self._both().pages()]
        assert "The index" not in titles
        assert "Main menu" in titles

    def test_and_the_reachable_one_still_is(self) -> None:
        assert "The time now" in [about.title for about in self._both().pages()]

    async def test_which_agrees_with_what_answers(self) -> None:
        both = self._both()
        assert both.route(PageAddress("1")) is not None
        assert both.describe(PageAddress("1")) == "Main menu"


class TestAMountMustCoverWhatItMounts:
    """A prefix says who answers; it is not stripped from the address.

    So a mounted service has to already number its pages inside the prefix it
    is mounted at. Mounting a service numbered 1 to 9 under `6` forwards it
    nothing, ever -- and every symptom of that shows up somewhere else: a
    keyword landing on the host's page of the same number, a contents entry for
    a page nobody can reach, a history that cannot name where it has been.

    Cheaper to refuse it than to debug it on a 1200 baud line.
    """

    def test_a_service_numbered_outside_the_prefix_is_refused(self) -> None:
        inner = Sextile(pages=[PageRoute("1", _nothing, name="main", title="Index")])
        with pytest.raises(ValueError, match="6"):
            Sextile(pages=[Mount("6", inner)])

    def test_one_numbered_inside_it_is_not(self) -> None:
        inner = Sextile(
            pages=[PageRoute("62{n:int}", _nothing, name="thing", title="A thing")]
        )
        app = Sextile(pages=[Mount("6", inner)])
        assert app.route(PageAddress("62489493")) is not None

    def test_the_catch_all_mount_takes_anything(self) -> None:
        #  `""` is the documented way to assemble a service out of one
        #  application, and every page number begins with it.
        inner = Sextile(pages=[PageRoute("1", _nothing, name="main", title="Index")])
        app = Sextile(pages=[Mount("", inner)])
        assert app.route(PageAddress("1")) is not None

    def test_a_page_that_says_nothing_about_itself_is_not_checked(self) -> None:
        #  Only advertised pages can be checked: a title frame declares no
        #  title, so nothing here knows it exists. That is a limit worth
        #  stating rather than a rule worth bending.
        inner = Sextile(pages=[PageRoute("0", _nothing, name="title")])
        assert Sextile(pages=[Mount("6", inner)]) is not None
