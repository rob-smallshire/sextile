...
class TestWhatTheFrameworkSaysForItself:
    ...
    def test_the_way_home_is_called_what_the_service_calls_it(self) -> None:
        ...
        assert "back to the main menu" in text_of(a_guide(home_label="main menu"))
        assert "back to the main index" in text_of(a_guide(home_label="main index"))
    ...
    def test_and_dropped_where_a_service_has_filled_the_frame(self) -> None:
        ...
        #  what it asked for.
        crowded = a_guide(moving_rows=[GuideRow(f"K{n}", f"does {n}") for n in range(9)])
    ...
    def test_and_kept_where_they_have_left_room(self) -> None:
        assert "previous" in text_of(a_guide(moving_rows=[GuideRow("A-Z", "type")]))


class TestWhatAServiceAddsToIt:
    def test_its_own_moving_keys_join_the_first_frame(self) -> None:
        guide = a_guide(moving_rows=[GuideRow("A-Z", "type into a search field")])
        assert "type into a search field" in text_of(guide)

    def test_and_its_own_pages_the_second(self) -> None:
        guide = a_guide(asking_rows=[GuideRow("*95#", "what the pictures mean")])
    ...
    def test_the_column_is_one_width_for_the_whole_guide(self) -> None:
        ...
        guide = a_guide(asking_rows=[GuideRow("*1234567890#", "a very long number")])
